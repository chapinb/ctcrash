...
def read_csv(csv_file: Path, columns: list[str] | None = None, **kwargs) -> pd.DataFrame:
...
def create_map(df):
    # Create a base map
    base_map = folium.Map(location=[df['Latitude'].mean(), df['Longitude'].mean()], zoom_start=13)
...
class CTCrashData:
    ...
    def load_csv(self, csv_file: Path, columns: list[str] | None = None, **kwargs) -> None:
    ...
    def create_pivot_table(self, index: str, columns: str, values: str = 'CrashId') -> pd.DataFrame:
